...
class DB
{
    ...
    public static function ConnectMysql($silent_errors = false)
    {
        ...
        try {
            ...
            if ($GLOBALS[self::PDO_DB]) {
                ...
                //$GLOBALS[self::PDO_DB]->setAttribute(PDO::ATTR_EMULATE_PREPARES, false);
                $GLOBALS[self::PDO_DB]->setAttribute(PDO::ATTR_ORACLE_NULLS, PDO::NULL_TO_STRING);
            ...
            if (!empty($placeholders) &&
                ...
                foreach ($placeholders as $field => $value) {
                    ...
                    if (is_int($value)) {
                        ...
                        $value = isset($value) && strlen($value) > 0 ? $value : null ;                         
                        $datatype = PDO::PARAM_STR;
                    }
    ...
    public static function Insert($table, $values, $debug = false)
    {
        ...
        $tabla = substr($table, ($pos = strpos($table, '.')) === false ? 0 : $pos + 1);

        $sql = " desc " . trim($tabla) . " ";
        
        $columnas = self::Query($sql);
        $columnas =  array_column($columnas, 'field');
        $columnas = array_flip($columnas);

        $values = array_intersect_key(array_change_key_case($values, CASE_LOWER), array_change_key_case($columnas, CASE_LOWER));
    ...
    /**
     * Inserts or update a new record into a table  using PDO
     *
     * @param string $table Table name
     * @param array $values Associative array containing the fields and values
     * array('name' => 'Cathy', 'city' => 'Cardiff')
     ...
     * @return boolean Returns the last inserted ID or TRUE otherwise FALSE
     */
    public static function InsertOrUpdate($table, $values, $debug = false)
    {
   ...
   //die($sql);
    ...
    public static function ConvertQueryToSimpleArray($array, $value_field, $key_field = false)
    {
        ...
        foreach ($array as $element) {
            ...
            if ($key_field) {
                // Add this key
                $return[$element[$key_field]] = strtolower($element[$value_field]);
            } else {
                // No key field
                // Append to the array
                $return[] = strtolower($element[$value_field]);
