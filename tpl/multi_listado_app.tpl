...
<script>
  ...
  function iFnc(e, t) {
    ...
    n += "<table  class='table table-bordered table-hover table-collapsed'><thead>" + s + "</thead><tbody>" + r + "</tbody></table>";
    return n;
  }
   ...
   $.each(json, function (i, row) {
      ...
      $.each(row, function (e,t) {  
        if ($.isArray(t) && t.length > 0) {
            n = "<tr class='collapse1 collapsed'><td colspan='" + v + "'><div id='accordion'><div id='" + s + "' class=''>" + iFnc(e, t) + "</div></div></td></tr>"      
        }      
        var findItem = $.grep(titulos, function (i) { return (e == i.name)  });      

        $.each(findItem, function (f, g) {       
              clase = '';
              if (typeof g.opciones !== 'undefined' && typeof g.opciones.atr !== 'undefined' && typeof g.opciones.atr.class !== 'undefined') {
                clase = g.opciones.atr.class;
              }            
              if(e == g.name){              
                r += '<td id="' + g.name + '" data-label="' + g.label + '" class="' + clase + '"  ' + g.hidden + ' >' + t + '</td>'
              }          
        });
      });
      bodyRows += r + n;     
